...
if __name__ == '__main__':
    ...
    predictions = [None for k in range(reps)]
    for r in range(reps):
        for subsetSize in numFeatures:
            ...
            predictions[r] = AveragePredictions(trees, S, T)

    groundTruth = S['label'].tolist()
    # Compute bias term
    ## Pick first tree in each baggs
    ## Compute the predictions of those first 100 trees
    prediction = [predictions[i][0] for i in range(reps)]
    ## Take the average
    total = [0 for r in range(len(S.index))]
    for r in range(reps):
        total = np.add(total, prediction[r])
    ...
    finalPredictions = [predictions[r][-1] for r in range(reps)]
    total1 = [0 for i in range(5000)]
    for t in range(reps):
        total1 = np.add(total1, finalPredictions[t])
    average1 = np.divide(total1, reps)
    bias1 = (average1 - groundTruth) ** 2
    sum1 = 0
    for t in range(reps):
        sum1 += finalPredictions[t] - average1
